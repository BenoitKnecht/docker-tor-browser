...
RUN wget https://www.torproject.org/dist/torbrowser/${TOR_VERSION}/tor-browser-linux64-${TOR_VERSION}_en-US.tar.xz
RUN wget https://www.torproject.org/dist/torbrowser/${TOR_VERSION}/tor-browser-linux64-${TOR_VERSION}_en-US.tar.xz.asc
RUN gpgv --keyring /tmp/${TOR_PGP_KEYID}.gpg tor-browser-linux64-${TOR_VERSION}_en-US.tar.xz.asc tor-browser-linux64-${TOR_VERSION}_en-US.tar.xz

WORKDIR /opt
RUN tar Jxvf /tmp/tor-browser-linux64-${TOR_VERSION}_en-US.tar.xz
